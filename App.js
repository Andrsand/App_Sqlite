...
export default function App() {
  ...
  const [isLoading, setIsLoading] = useState(true); // для вывода сообщения загружается база или нет
  const [names, setNames] = useState([]);
  const [currentName, setCurrentName] = useState(undefined);  // поле для редактирования имен с выводом текущего имени
  ...
  if (isLoading) {
    return (
      <View style={styles.container}>
        <Text>Loading names...</Text>        {/*вывод из бд */}
      </View>
    );
  }
  // функция добавления имени
  const addName = () => {
    db.transaction(tx => {
  ...
  return (
    <View style={styles.container}>
      <TextInput value={currentName} placeholder='name' onChangeText={setCurrentName} /> {/*поле редактирование текущего имени */}
      <Button title="Add Name" onPress={addName} />  {/*конопка добавления имени */}
      {showNames()}
